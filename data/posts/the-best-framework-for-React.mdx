...
One of the building blocks of web apps is Routing. I worked on many React apps in the past, before NextJs, people kinda "freestyle" on choosing the project's folder structure and naming conventions etc... Every project have a different ways to do routing, file structures since there is really no universal standard, 
which is sometimes hard to read.

Now in Nextjs, they have a features that if you want to make a page you have to put it under `/pages` folder and export it as default.
...
Are there any alternatives to Nextjs ? Yes, checkout [Remix.run](https://remix.run). Remix is a relatively new framework compared to Nextjs, and it's built on top ReactRouterV6. I've tried both, but I prefered Nextjs more for its simplicity.
